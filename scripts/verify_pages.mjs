...
const networkTimeout=Number(process.env.RESOURCE_TIMEOUT_MS||30000);
page.setDefaultTimeout(networkTimeout);
page.setDefaultNavigationTimeout(networkTimeout);
const getResource=url=>page.request.get(url,{timeout:networkTimeout});
const errors=[],checks=[];
const recordCheck=label=>{checks.push(label);console.log('Passed: '+label);};
...
try{
 ...
 const buildResponse=await getResource(root+'build.json');assert.equal(buildResponse.status(),200);
 const build=await buildResponse.json();
 if(process.env.EXPECTED_COMMIT)assert.equal(build.commit,process.env.EXPECTED_COMMIT);
 recordCheck('站点首页、项目导航与发布版本');
 ...
 for(const suffix of ['code/gallery-fixtures/design.json','notes/graph-types.md','notes/comparison.md','assets/capability-map.svg','assets/capability-map.png','README.md']){
  const r=await getResource(root+'001-understand-anything/'+suffix);assert.equal(r.status(),200,suffix);
 }
 recordCheck('样本、证据说明、研究文档与高清图可访问');
 ...
 if(build.projects.includes('002-claude-code-best-practice')){
  ...
  for(const suffix of ['assets/understanding-map.svg','assets/understanding-map.png','README.md','notes/research.md','notes/evidence/experiments.json','licenses/upstream.LICENSE']){
   assert.equal((await getResource(root+'002-claude-code-best-practice/'+suffix)).status(),200,suffix);
  }
  ...
  recordCheck('002 引导图与定位摘要、缩放、文档和高清资源、成功失败流程、刷新与手机布局');
 }
 if(build.projects.includes('003-frontend-design-toolkit')){
  ...
  for(const suffix of ['README.md','THIRD_PARTY_NOTICES.md','notes/research.md','notes/usage.md','notes/evidence/sources.json','assets/cover.png','assets/understanding-map.svg','assets/understanding-map.png']){
   assert.equal((await getResource(root+'003-frontend-design-toolkit/'+suffix)).status(),200,suffix);
  }
  await page.setViewportSize({width:390,height:844});
  assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));
  recordCheck('003 总览图及缩放、同一需求与四种资源的使用关联、能力筛选、六步流程、主题变量、证据刷新、文档及手机布局');
 }
