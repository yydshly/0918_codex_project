...
| 状态 | 静态研究与中文网页完成；本地网页已验证，未发布公网，未验证外链服务 |
...
未逐项验证外部页面的状态、全文、收费规则或更新日期；未使用融资计算器，未联系投资人，未开展客户访谈或招聘。上游没有可运行应用；本项目额外制作了中文导航网页并保存真实截图，不将其描述成上游软件。未发布到公网，也未登记计划中的部署地址。
...
浏览器验证已覆盖六组筛选及刷新、细分类型切换、图片资源、放大缩小、手机滚动和返回资料导航，并保留此前完整目录与搜索检查。证据继续记录在 [browser.json](evidence/browser.json) 与 [integration.json](evidence/integration.json)。未发布到公网。
