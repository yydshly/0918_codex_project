...
try{
 ...
 const entry=manifest.find(x=>x.slug==='011-selfteaching');assert.ok(html.includes(entry.summary));
 for(const item of manifest){assert.ok(html.includes(item.slug+'/'));await get(item.slug+'/')}
 const prefix='011-selfteaching/';
 ...
 for(const term of ['能力与方向','方法与技巧','提供哪些资料','对我的意义','重读与整理概念','Jupyter Notebook'])assert.ok(source.includes(term));
